<?php
    ...
    /**
     * Classe Aluno - baseado no modelo Active Record (Simplificado) 
    ...
    class Aluno{
        ...
        public function save(){
            ...
            if (!isset($this->id)) {
                ...
                $query = "UPDATE aluno SET ".implode(', ', $definir)." WHERE id='{$this->id}';";
            }
        ...
        public static function all(){
            ...
            $result  = array();
            if ($stmt->execute()) {
                while ($rs = $stmt->fetchObject(Aluno::class)) {
                    $result[] = $rs;
                }
            }
            if (count($result) > 0) {
        ...
        public static function count(){
            $conexao = Conexao::getInstance();
            $count   = $conexao->exec("SELECT count(*) FROM aluno;");
            if ($count) {
                return (int) $count;
            }
            return false;
        ...
        public static function find($id){
            ...
            if ($stmt->execute()) {
                if ($stmt->rowCount() > 0) {
                    $resultado = $stmt->fetchObject('Aluno');
                    if ($resultado) {
                        return $resultado;
                    }
                }
        ...
        public static function destroy($id){
            $conexao = Conexao::getInstance();
            if ($conexao->exec("DELETE FROM aluno WHERE id='{$id}';")) {
                return true;
            }
            return false;
        }
